    ...
    <div class="flex items-center justify-between h-16 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex items-center space-x-4">
            ...
            <div class="relative" x-data="{ open: false }" x-init="open = false">
                <button @click="open = !open" @click.stop class="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors duration-200">
                    <img class="h-8 w-8 rounded-full" src="{{ Auth::user()->profile_picture ? (str_starts_with(Auth::user()->profile_picture, 'images/') ? asset(Auth::user()->profile_picture) : (str_starts_with(Auth::user()->profile_picture, 'http') ? Auth::user()->profile_picture : asset('storage/' . Auth::user()->profile_picture))) : asset('images/avatars/avatar-default.svg') }}" alt="{{ Auth::user()->name }}">
                ...
                <div x-show="open" 
                    ...
                    <a href="{{ route('profile.edit') }}#password-section" class="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700">
                        <svg class="w-4 h-4 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
