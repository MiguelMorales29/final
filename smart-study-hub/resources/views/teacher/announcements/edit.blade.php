<x-teacher-layout>
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <p class="mt-2 text-gray-600 dark:text-gray-400">Update your announcement</p>
        </div>

        <!-- Form -->
        <form action="{{ route('teacher.announcements.update', $announcement) }}" method="POST" class="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <select id="event_type" name="event_type" required
                        class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    <option value="other" {{ old('event_type', $announcement->event_type) == 'other' ? 'selected' : '' }}>Other</option>
                    <option value="quiz" {{ old('event_type', $announcement->event_type) == 'quiz' ? 'selected' : '' }}>Quiz</option>
                    <option value="exam" {{ old('event_type', $announcement->event_type) == 'exam' ? 'selected' : '' }}>Exam</option>
                    <option value="assignment" {{ old('event_type', $announcement->event_type) == 'assignment' ? 'selected' : '' }}>Assignment</option>
                </select>
                @error('event_type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6" id="custom_event_type_container" style="display: {{ old('event_type', $announcement->event_type) == 'other' ? 'block' : 'none' }};">
                ...
                <input type="text" id="custom_event_type" name="custom_event_type" value="{{ old('custom_event_type', $announcement->custom_event_type) }}"
                       class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500"
            ...
            <div class="mb-6">
                ...
                <input type="datetime-local" id="event_date" name="event_date" value="{{ old('event_date', $announcement->event_date ? $announcement->event_date->format('Y-m-d\TH:i') : '') }}"
                       class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                @error('event_date')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                <div class="flex items-center">
                    <input type="checkbox" id="show_on_calendar" name="show_on_calendar" value="1" {{ old('show_on_calendar', $announcement->show_on_calendar) ? 'checked' : '' }}
